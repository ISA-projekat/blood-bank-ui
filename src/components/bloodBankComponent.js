...
export default class BloodBankComponent extends Component {
  ...
  renderBloodBanks = (bloodBanks) => {
    ...
    if (!bloodBanks || bloodBanks.length === 0) {
      return <div>There are no blood banks that fulfill this criteria</div>;
    }

    for (let bb of bloodBanks) {
      result.push(
        <li key={bb.id}>
          Name: {bb.name}
          <br />
          Rating: {bb.rating} <br />
          Address: {bb.address.street} {bb.address.number}, {bb.address.city},{" "}
          {bb.address.country}
        </li>
      );
    }
  ...
  render() {
    return (
      <div>
        ...
        <div>
          <ul>{this.renderBloodBanks(this.state.bloodbanks)}</ul>
        </div>
      </div>
    );
  }
