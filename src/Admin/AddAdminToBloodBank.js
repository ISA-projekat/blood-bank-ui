...
import { getAvailableAdministators } from "../services/user/UserService";
...
const AddAdminToBloodBank = () => {
  ...
  const fetchData = () => {
    getAvailableAdministators().then((response) => {
    ...
    <div>
      <div className="register">
        <NavLink to="/admin/register">
          <button>Register new administrator</button>
        </NavLink>
      </div>
      <div className="outterDiv">
        ...
        <div>
          ...
          <select
            onChange={(event) => changeAdmin(event.target.value)}
            value={adminId}
          >
